...
export default function GameViewPage() {
	...
	const [delayedState, setDelayedState] = useState<ParsedGameState | null>(null);
	...
	useEffect(() => {
		...
		async function load() {
			try {
				...
				if (isTestGame) {
					console.log(`[TEST GAME] Fetching with timestamp: ${testGameTimestamp}, URL: ${url}`);
				}
				...
				if (data?.state) {
					...
					console.log(`[LIVE] Update #${liveUpdateCount + 1} received at ${new Date(now).toLocaleTimeString()}.${now % 1000}`);
				}
				setError(null);
			} catch {
	...
	useEffect(() => {
		const interval = setInterval(() => {
			...
			if (eligibleStates.length > 0) {
				...
				if (!prevState || targetState.state.clock !== prevState.clock || 
					...
					console.log(`[DELAYED] Update #${delayedUpdateCount + 1} shown (${ageSeconds}s old) at ${new Date(now).toLocaleTimeString()}`);
				}
			}
		}, 100);
	...
	function registerPrediction(pred: { ts: number; period?: number | string | null; clock?: string }) {
		if (!predictionWindowActive) return;
	...
	function detectNewShot(gameState: ParsedGameState) {
		...
		console.log(`Shot detected! Will show popup in ${popupDelay/1000}s, countdown for 3s`);
		...
		setTimeout(() => {
			...
			setTimeout(() => {
				...
				// Check predictions - only award points if user made gesture during countdown
				const isMade = lastShot.shotResult?.toLowerCase().includes('made');
				const hadPrediction = predictionsRef.current.length > 0;
				
				if (hadPrediction && isMade) {
					const earned = 10;
			...
			try {
				...
				content.innerHTML = `
					<div class="card">
						...
						<div class="value timestamp">${now.toLocaleTimeString()}.${now.getMilliseconds().toString().padStart(3, '0')}</div>
					</div>
					<div class="card">
						<div class="label">Period ${liveState.period ?? '-'} • ${liveState.clock ?? '--:--'}</div>
						<div class="grid">
							<div>
					...
					${liveState.lastShot ? `
						<div class="card" style="border-left: 4px solid ${liveState.lastShot.shotResult?.toLowerCase().includes('made') ? '#10b981' : '#ef4444'}">
							<div class="label">Last Shot • ${now.toLocaleTimeString()}</div>
							<div class="value">${liveState.lastShot.playerName} (${liveState.lastShot.teamTricode})</div>
							<div style="margin-top: 8px; color: ${liveState.lastShot.shotResult?.toLowerCase().includes('made') ? '#10b981' : '#ef4444'}; font-weight: bold;">
								${liveState.lastShot.shotResult} • ${liveState.lastShot.shotType || ''} • ${liveState.lastShot.points || 0} pts
							</div>
							${liveState.lastShot.description ? `<div style="margin-top: 4px; font-size: 12px; opacity: 0.7;">${liveState.lastShot.description}</div>` : ''}
						</div>
					` : ''}
					${liveState.ballHandler ? `
						<div class="card">
							<div class="label">Ball Handler</div>
							<div class="value">${liveState.ballHandler.name} (${liveState.ballHandler.teamTricode})</div>
							${liveState.ballHandler.liveStats ? `
								<div style="margin-top: 8px; font-size: 14px;">
									PTS: ${liveState.ballHandler.liveStats.points ?? 0} • 
									FG: ${liveState.ballHandler.liveStats.fieldGoalsMade ?? 0}/${liveState.ballHandler.liveStats.fieldGoalsAttempted ?? 0} 
									(${liveState.ballHandler.liveStats.fieldGoalsAttempted > 0 ? Math.round((liveState.ballHandler.liveStats.fieldGoalsMade / liveState.ballHandler.liveStats.fieldGoalsAttempted) * 100) : 0}%)
								</div>
							` : ''}
						</div>
					` : ''}
					${liveState.lastAction ? `
						<div class="card">
							...
							<div style="font-size: 14px; margin-top: 4px;">
								<span style="font-weight: bold;">${liveState.lastAction.playerName || 'Unknown'}</span>
								${liveState.lastAction.teamTricode ? `<span style="opacity: 0.7;"> (${liveState.lastAction.teamTricode})</span>` : ''}
								${liveState.lastAction.actionType ? `<span style="margin-left: 8px; background: rgba(255,255,255,0.1); padding: 2px 6px; border-radius: 4px; font-size: 11px;">${liveState.lastAction.actionType}</span>` : ''}
								${liveState.lastAction.shotResult ? `<span style="margin-left: 4px; font-weight: bold;">${liveState.lastAction.shotResult}</span>` : ''}
							</div>
							${liveState.lastAction.description ? `<div style="margin-top: 4px; font-size: 12px; opacity: 0.6;">${liveState.lastAction.description}</div>` : ''}
						</div>
					` : ''}
					${recentActions.length > 0 ? `
						<div class="card">
							...
							<div style="max-height: 300px; overflow-y: auto; margin-top: 8px;">
								${recentActions.slice(0, 8).map((act, i) => `
									<div style="padding: 8px; margin-bottom: 4px; background: rgba(255,255,255,0.05); border-radius: 4px; font-size: 12px; border-left: 2px solid ${act.shotResult ? (act.shotResult.toLowerCase().includes('made') ? '#10b981' : '#ef4444') : '#666'}">
										...
										<div style="opacity: 0.8;">
											${act.teamTricode ? `<span style="opacity: 0.6;">${act.teamTricode}</span> • ` : ''}
											${act.actionType ? `<span style="background: rgba(255,255,255,0.1); padding: 1px 4px; border-radius: 3px;">${act.actionType}</span>` : ''}
											${act.shotResult ? `<span style="margin-left: 4px; font-weight: bold; color: ${act.shotResult.toLowerCase().includes('made') ? '#10b981' : '#ef4444'}">${act.shotResult}</span>` : ''}
										</div>
									</div>
								`).join('')}
							</div>
			...
			<div className="glass rounded-xl p-4 space-y-4">
				...
				{isTestGame && (
					<div className="bg-blue-500/20 border border-blue-500/50 rounded-lg p-3">
						<div className="flex items-center justify-between mb-2">
							<span className="text-xs text-blue-400 font-semibold">TEST GAME TIMELINE</span>
							<span className="text-sm font-bold">Action {testGameTimestamp + 1}/7</span>
						</div>
						<input
							type="range"
							min="0"
						...
						<div className="text-xs text-blue-300 mt-2">
							Scrub through test game actions to see shot detection and predictions
						</div>
					</div>
				)}
				...
				<div className="bg-white/5 rounded-lg p-3">
					...
					<div className="text-xs opacity-60 mt-1">
						Popup appears {Math.max(0, streamDelay - 3)}s before shot on your stream
					</div>
				</div>
						...
						<div className="mt-2 space-y-2">
							<div className="flex items-center justify-between text-sm">
								...
								<span className="font-bold text-lg">
									{state.ballHandler.liveStats.fieldGoalsAttempted > 0
										? Math.round((state.ballHandler.liveStats.fieldGoalsMade / state.ballHandler.liveStats.fieldGoalsAttempted) * 100)
										: 0}%
								</span>
							</div>
							<div className="grid grid-cols-4 gap-2 text-xs">
								<div>
									...
									<div className="font-bold">{state.ballHandler.liveStats.points ?? 0}</div>
								</div>
								<div>
									<div className="opacity-60">FG</div>
									<div className="font-bold">{state.ballHandler.liveStats.fieldGoalsMade ?? 0}/{state.ballHandler.liveStats.fieldGoalsAttempted ?? 0}</div>
								</div>
								<div>
									<div className="opacity-60">3PT</div>
									<div className="font-bold">{state.ballHandler.liveStats.threePointersMade ?? 0}/{state.ballHandler.liveStats.threePointersAttempted ?? 0}</div>
								</div>
								<div>
									<div className="opacity-60">AST</div>
									<div className="font-bold">{state.ballHandler.liveStats.assists ?? 0}</div>
								</div>
							</div>
						</div>
					)}
					...
					<div className="bg-gradient-to-r from-orange-500/20 to-red-500/20 rounded-lg p-3">
						...
						{state.shooter.liveStats && (
							<div className="mt-2 grid grid-cols-3 gap-2 text-xs">
								<div>
									<div className="opacity-60">PTS</div>
									<div className="font-bold text-sm">{state.shooter.liveStats.points ?? 0}</div>
								</div>
								<div>
									<div className="opacity-60">FG%</div>
									<div className="font-bold text-sm">
										{state.shooter.liveStats.fieldGoalsAttempted > 0
											? Math.round((state.shooter.liveStats.fieldGoalsMade / state.shooter.liveStats.fieldGoalsAttempted) * 100)
											: 0}%
									</div>
								</div>
								<div>
									...
									<div className="font-bold text-sm">
										{state.shooter.liveStats.threePointersAttempted > 0
											? Math.round((state.shooter.liveStats.threePointersMade / state.shooter.liveStats.threePointersAttempted) * 100)
											: 0}%
									</div>
								</div>
							</div>
