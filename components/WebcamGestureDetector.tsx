    ...
    <div className="flex flex-col gap-5">
        ...
        <div className="pointer-events-none absolute inset-0">
          {activeLabels.map((label, index) => {
            ...
            return (
              <div
                key={label}
                className={`absolute top-3 rounded-md border border-white/10 bg-black/75 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.35em] text-white shadow-[0_0_12px_rgba(28,255,176,0.25)] ${position}`}
              >
                {label}
              </div>
            );
      ...
      <div className="flex flex-col gap-2 border border-white/15 bg-black/40 p-4 text-sm text-slate-200 shadow-[0_18px_38px_rgba(0,0,0,0.55)]">
        <label className="font-semibold uppercase tracking-[0.3em] text-emerald-100">
          Active Bettors: {playerCount === 3 ? "3+" : playerCount}
        </label>
        <input
          type="range"
          min={1}
